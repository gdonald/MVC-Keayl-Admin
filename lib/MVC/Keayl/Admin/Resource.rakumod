...
method attribute(Str:D $name, :&display, Str :$format, Bool :$html = False --> ::?CLASS) {
  reject-unknown(%_, "attribute '$name'");

  @!attributes.push: MVC::Keayl::Admin::Attribute.new(:$name, :&display, :$format, :$html);

  self
}

method field(Str:D $name, Str :$as = 'string', Str :$label, :&collection, Str :$hint, Str :$placeholder, Bool :$multiple = False, Int :$rows, :&value --> ::?CLASS) {
  reject-unknown(%_, "field '$name'");

  die "unknown field type '$as' for field '$name'" unless FIELD-TYPES{$as};

  @!fields.push: MVC::Keayl::Admin::Field.new(:$name, :$as, :$label, :&collection, :$hint, :$placeholder, :$multiple, :$rows, :&value);
